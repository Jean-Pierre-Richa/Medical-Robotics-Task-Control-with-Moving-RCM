...
void DYN::RESIDUAL::NewEul_Aux(Params& kinParams, dynamicParams& dynParams, residualParams& residual, inertiaMat& iMat){

  NewEulForw(kinParams, dynParams, residual, iMat);

  residual.tau = NewEulBack(kinParams, dynParams, residual, iMat);

}

void DYN::RESIDUAL::NewEulForw(Params& kinParams, dynamicParams& dynParams, residualParams& residual, inertiaMat& iMat){

  MatXd T(4,4);
  T.setZero();
  T = KukaKinematics(kinParams, dynParams, residual, iMat);
  ...
  vec3d curr_wmd(0,0,0);
   ...
   for (int i=0; i<dynParams.N; i++){
      ...
      */
     vec3d rici(-dynParams.lengths[i]/2, 0, 0);
     /*
...
vecxd DYN::RESIDUAL::NewEulBack(Params& kinParams, dynamicParams& dynParams, residualParams& residual, inertiaMat& iMat){
  ...
  for (int i=dynParams.N-1; i>-1; i--){
     ...
     */
    // cout << "masses " << dynParams.masses[i] << endl;
    curr_f = R*prev_f + (dynParams.masses[i]*pcdd);
    /*
    ...
    I_tensor = MatXd::Identity(3,3)*residual.I[i] + dynParams.masses[i] * ((Skew.transpose())*Skew);
    /*
     * \Assuming that the motor is at the origin
     */
    MatXd Im_tensor(3,3);
    Im_tensor = MatXd::Identity(3,3)*residual.Im+residual.mm*((Skew.transpose())*Skew);
    /*
     * \Equation 7.113 (Siciliano). Depending on the book edition, there might
     * \be a typo error with a double cross(w,I_tensor*w)
     */
    vec3d prev_u_(0,0,0);
    prev_u_ = R*prev_f;
    vec3d I_tensor_(0,0,0);
    I_tensor_ = I_tensor*w;
    vec3d Im_tensor_(0,0,0);
    Im_tensor_ = Im_tensor*w;
    curr_u = -curr_f.cross(rij+rici)+(R*prev_u)+(prev_u_).cross(rici)+(I_tensor*wd)+w.cross(I_tensor_)+residual.kri*qdd[i+1]*(Im_tensor*zm)+residual.kri*qd[i+1]*((Im_tensor_).cross(zm));
    /*
     * \Equation 7.114 for a revolute joint(Siciliano)
     */
    vec3d Imwmd_tensor_(0,0,0);
    Imwmd_tensor_ = Im_tensor*wmd;
    // cout << "first " << curr_u.transpose()*(Rt*z0)+residual.kri*Imwmd_tensor_.transpose()*zm <<endl;
    // cout << "second" << dynParams.fv*qd[i]+dynParams.fc*sign(qd[i]) << endl;
    // cout << qd[i] << endl;
    // cout << "fv*qd[i] " << dynParams.fv*qd[i] << endl;

    MatXd tau_part_1(1,1);

    tau_part_1 << (curr_u.transpose()*(Rt*z0)+(residual.kri*Imwmd_tensor_.transpose()*zm));

    tau[i] = tau_part_1(0,0)+(dynParams.fv*qd[i]+dynParams.fc*sign(qd[i]));
...
MatXd DYN::RESIDUAL::KukaKinematics(Params& kinParams, dynamicParams& dynParams, residualParams& residual, inertiaMat& iMat){
  ...
  double d1, d3, d5, d7;
  d1 = dynParams.lengths[0] + dynParams.lengths[1];
  d3 = dynParams.lengths[2] + dynParams.lengths[3];
  d5 = dynParams.lengths[4] + dynParams.lengths[5];
  d7 = dynParams.lengths[6] + kinParams.ee_length;
